...
| AVO-004.7-C8 | Hosted organization/queue/release gate | AVO-004.7-C7 | Terra-approved local Wave 1/2 foundations plus hosted workflow/configuration and operator-controlled state root only after explicit authority. | Phase 1 snapshot is accepted at `daeff01` through `f38840d` with 52 focused tests. The pure Phase 2 parser gate is Terra-approved at `e154726` with 19 parser tests / 71 combined focused checks for bounded raw effective/resolved rulesets, conditions/rule multisets, strict required contexts/App configuration, and queue parsing. This remains parser-only: no transport integration, live/CLI execution, authority, or readiness; the next local leaf is atomic authenticated Phase 2 snapshot composition. Final protocol still requires an organization-owned required merge queue with max entries per group=1, a separate isolated issuer (not App 15368), exclusive controller create/delete authority for `avo/main-rollback/*`, a fresh hosted main rollback drill, then ledger activation and 12 consecutive eligible successes with 0 failures/boundary violations. Until those prerequisites exist this leaf is blocked and may not mutate current `main`. |
